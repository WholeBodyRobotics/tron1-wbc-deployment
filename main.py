...
if __name__ == '__main__':
    ...
    robot_ip = os.getenv("TRON1_IP", "10.192.1.2")
    ...
    if not robot.init(robot_ip):
        sys.exit()
    print(f"MotorNumber: {robot.getMotorNumber()}")

    # Start the controller immediately after the robot connection succeeds.
    ...
    if robot_type.startswith("PF"):
      ...
      controller.run()
    elif robot_type == "SF_TRON1A_ARX5ARM":
      controller = controllers.SolefootArmController(f'{os.path.dirname(os.path.abspath(__file__))}/controllers/model', robot, robot_type, rl_type, start_controller)
      controller.run()
